...
MetaballEnemy::MetaballEnemy(Eigen::Vector3f center, int numBlobs, int radius)
{
	...
	this->speed = DEFAULT_BLOB_CREATURE_SPEED;
...
void MetaballEnemy::moveMetaballEnemy()
{
	...
	else if (actionState == CHARACTER_DETECTED_ACTION_STATE)
	{
		//Appropriate Actions TODO
	}
...
{	
	if ((v.x - radius) < -BLOB_CREATURE_MOVE_BOUNDARY || (v.x + radius) > BLOB_CREATURE_MOVE_BOUNDARY ||
		(v.y - radius) < -BLOB_CREATURE_MOVE_BOUNDARY || (v.y + radius) > BLOB_CREATURE_MOVE_BOUNDARY ||
		(v.z - radius) < -BLOB_CREATURE_MOVE_BOUNDARY || (v.z + radius) > BLOB_CREATURE_MOVE_BOUNDARY)
	{
		return true;
	}
	else
...
void MetaballEnemy::checkToFire()
{
	...
	{
		Vertex tempProjectileDirection = generateRandomNormalizedDirection();
		Vector3f projectileDirection;
		projectileDirection(0) = tempProjectileDirection.x;
		projectileDirection(1) = tempProjectileDirection.y;
		projectileDirection(2) = tempProjectileDirection.z;
...
void MetaballEnemy::checkToUpdate()
{
	for (int i = 0; i < numBlobs; i++)
	{
		...
		float nextX = curBlob.center.x + direction.x * DEFAULT_BLOB_SPEED;
		float nextY = curBlob.center.y + direction.y * DEFAULT_BLOB_SPEED;
		float nextZ = curBlob.center.z + direction.z * DEFAULT_BLOB_SPEED;
		...
		while (maxDistance > maxRadius)
		{
			direction = generateRandomNormalizedDirection();
			
			nextX = curBlob.center.x + direction.x * DEFAULT_BLOB_SPEED;
			nextY = curBlob.center.y + direction.y * DEFAULT_BLOB_SPEED;
			nextZ = curBlob.center.z + direction.z * DEFAULT_BLOB_SPEED;
			
			maxDistance = sqrt(pow(nextX, 2) + pow(nextY, 2) + pow(nextZ, 2)) + this->maxBlobRadius;
		}
		
		while (curBlob.pastCenter && (minDistance < MIN_DISTANCE_FROM_CENTER))
		{
			direction = generateRandomNormalizedDirection();
		...
		if (minDistance >= MIN_DISTANCE_FROM_CENTER) curBlob.pastCenter = true;
...
{	
	...
	glutSolidSphere(CENTER_SPHERE_RADIUS, CENTER_SPHERE_SLICES, CENTER_SPHERE_STACKS);
	glPopMatrix();
